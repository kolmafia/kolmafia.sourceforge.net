// ==UserScript==
// @name           Familiar Sorter
// @namespace      kolmafia
// @description    Sorts your familiars by experience, then by kills, then by name.
//
...
function sortFamiliars()
{
	...
	var experiencePattern = /<\/b>, the \d+-pound (.*) \(([\d,]+) exp, ([\d,]+) kills?\) <\/td>/i;
	...
	for ( var i = existingRows.length - 1; i >= 0; --i )
	{
		...
		var experienceMatcher = experiencePattern.exec( existingRows[i].innerHTML );

		familiarData[0].push( existingRows[i] );
		familiarData[1].push( experienceMatcher[1] );
		familiarData[2].push( parseInt( experienceMatcher[2].replace( /,/g, "" ) ) );
		familiarData[3].push( parseInt( experienceMatcher[3].replace( /,/g, "" ) ) );
	...
	while ( familiarData[0].length > 0 )
	{
		...
		for ( var i = 1; i < familiarData[1].length; ++i )
		{
			if ( familiarData[2][i] > familiarData[2][maximumRow] )
				maximumRow = i;
			else if ( familiarData[2][i] == familiarData[2][maximumRow] && familiarData[3][i] > familiarData[3][maximumRow] )
				maximumRow = i;
			else if ( familiarData[2][i] == familiarData[2][maximumRow] && familiarData[3][i] == familiarData[3][maximumRow] && familiarData[1][i] < familiarData[1][maximumRow] )
				maximumRow = i;
		}
		...
		familiarData[3].splice( maximumRow, 1 );
		maximumRow = 0;
	}
}
